...
function sshBaseArgs(timeoutMs) {
...
export async function diagnoseFabric({
  ...
  if (!tailscaleStatus.ok) {
    ...
    recommendedAction = `Start the tunnel on clinic-m2: ssh -N -L 11435:127.0.0.1:11434 ${sshTarget}`;
  } else if (!m2Ollama.ok) {
  ...
  return {
    ...
    target: { host: m5Host, sshUser, sshTarget },
    checks: {
      tailscaleCli: { ok: tailscaleStatus.ok, latencyMs: tailscaleStatus.latencyMs, error: tailscaleStatus.error },
      tailscaleToM5: { ok: tailscalePing.ok, latencyMs: tailscalePing.latencyMs, detail: tailscalePing.stdout, error: tailscalePing.error },
